Zwei der vorgestellten Spielideen werden mit Hilfe der benötigten Features als Android-App implementiert. Hierzu wird ein Framework entwickelt, das allgemein benötigte Funktionalitäten bereitstellt. Darauf aufbauend werden die beiden Beispielspiele umgesetzt.
...
\subsection*{Server}
Der implementierte Server arbeitet mit Java und ZeroMQ. Er öffnet zwei Kommunikationskanäle mit jedem Client. Der erste ist ein ZeroMQ-Request-Reply-Socket-Paar, über das die Endgeräte Nachrichten an den Server senden. Der zweite ist ein Publish-Subscribe-Socket-Paar, über das die Nachrichten an Gruppen von Endgeräten weitergeleitet werden. Eine Nachricht besteht aus drei Teilen: einer Adresse, dem Nachrichtentyp und einem serialisierten Objekt vom Typ TransferObject.
...
Bei einem Spielbeitritt einer laufenden Session, wird über den Server eine Anfrage des aktuellen Status des Spiels an alle momentanen Mitspieler gesendet. Die Mitspieler senden den Zustand des Spiels an den anfragenden Client zurück. Dieser beachtet nur die jeweils erste Nachricht und verwirft den Rest.
