...
\paragraph{Markieren:}
 Die Positionen aller mit Spieler sind allen Geräten bekannt, aber nicht alle sichtbar. Damit verbunden sind auch die Funktionalitäten des Markieren und Scannens. Für das Markieren werden jeweils die beiden Koordinaten des Markierenden (A) und des Markierten (B), sowie den Sichtkegel von A und eine Reichweite benötigt. 
Für den Sichtkegel benötigt man einen Orientierungswinkel ($\alpha$), welcher mittels der Sensoren für Orientierung (siehe Abschnitt \ref{orientierung}) ermittelt wird, und einen vorher festgelegten Winkel für den Sichtbereich ($\beta$). Nun wird der Winkel der beiden Punkte A und B ($\gamma$) bestimmt (siehe Abschnitt \ref{abstandsmessung}).
